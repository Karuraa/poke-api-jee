...
public class ShopController {
    ...
    @PostMapping
    public ResponseEntity<?> save(@RequestBody CreateShop shop) {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable String id, @RequestBody UpdateShop updateBody) {
        Shop shop = shopService.findById(id);
        if (shop == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        shopService.update(id, updateBody);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
